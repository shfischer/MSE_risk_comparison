### ------------------------------------------------------------------------ ###
### Extract uncertainty from SAM for simulations ####
### ------------------------------------------------------------------------ ###
#' Extract uncertainty from SAM for simulations
#'
#' This function takes the uncertainty from a SAM model fit and uses the 
#' variance-covariance matrix to simulate model estimates. This can be used in
#' an MSE to condition the operating model on a SAM stock assessment model fit.
#' 
#' Reported standard deviations are meant to be used on a log-scale, e.g. need
#' to be exponentiated (\code{exp(rnorm(..., mean = 0, sd = sd))})
#'
#' @param fit A SAM model fit of class \linkS3class{sam}.
#' @param n Number of replicates 
#' @param verbose Print messages when creating model estimates? Logical, 
#'   defaults to \code{FALSE}.
#' @param idx_cov Produce and return index covariance? Logical, 
#'   defaults to \code{TRUE}.
#' @param catch_est Return estimated catches (\code{TRUE}) or catch input 
#' data ({FALSE}, default).
#'
#' @return A list of \code{FLQuants}, each with n replicates (iterations): 
#' \itemize{
#'   \item stock.n: stock numbers at age for all assessment years, 
#'   \item harvest: Fishing mortality at age for all assessment years, 
#'   \item survey_catchability: survey catchability at age (time-invariant), 
#'   \item catch_sd: Standard error of observation uncertainty at age for ,
#'   \item survey_sd: Standard error of observation uncertainty at age age for 
#'   surveys (time-invariant), 
#'   \item survey_cov: Covariance of survey observations, 
#'   \item proc_error: Standard error of process error at age (time-invariant),
#'   \item catch_n: catch numbers at age for all assessment years.
#' }
#'   
#'
#' @export

setGeneric("SAM_sim", function(fit, 
                               n = 1000,
                               verbose = FALSE,
                               idx_cov = TRUE,
                               catch_est = FALSE) {
  standardGeneric("SAM_sim")
})
...
setMethod(f = "SAM_sim",
          ...
          definition = function(fit, 
            ...
            SAM_sim_run(fit = fit, n = n, verbose = verbose,
                        idx_cov = idx_cov, catch_est = catch_est)
...
SAM_sim_run <- function(fit, n = 1000, verbose = FALSE,
                        idx_cov = TRUE, ### get idx covariance
  ...
  if (isTRUE(verbose)) cat(paste0(., sep = "\n"))
