...
mpiexec R CMD BATCH --no-save --no-restore "--args use_MPI=TRUE n_workers=0 n_blocks=1 popSize=1000 maxiter=100 run=10 stock_id=$STOCK_ID n_iter=1000 n_yrs=20 scenario='multiplier' MP='hr' OM='baseline' ga_search=TRUE idxB_lag=TRUE idxB_range_3=TRUE exp_b=FALSE comp_b_multiplier=TRUE interval=TRUE multiplier=TRUE upper_constraint=1.2 lower_constraint=0.7 obj_fun='ICES' obj_yrs='11:20' stat_yrs='multiple' pen_neg=FALSE pen_max=1 pen_infl=0.06 pen_steep=1000 collate=TRUE add_suggestions=TRUE" $HOME/git/MSE_ple.27.7e/MP_run.R $HOME/reports/$PBS_JOBID.$PBS_ARRAY_INDEX.Rout
## $PBS_ARRAY_INDEX
